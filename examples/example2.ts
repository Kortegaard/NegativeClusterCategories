...
import * as n from "../src/NegativeCCDiagonalCollection"

// Tilt an proper abelian subcategory at a random torsion class
function randomTilt(alg: n.NCCDiagonalCollection){
    var T = ncc.findRandomTorsionFreeClass(alg)
    return ncc.tilt(alg, T)
}

var A: null | n.NCCDiagonalCollection = null
var B: null | n.NCCDiagonalCollection = null
...
for(var i = 0; i< 10000; i++){
    ...
    if(!SSA_SA_A.containsSet(alg3.diagonals)){ continue }
    if(!B_SB_SSB.containsSet(alg.diagonals)) { continue }

    // Ensure not boring
    var SA_A = ncc.extension(ncc.Sigma(alg), alg)
    if(SA_A.containsSet(alg3.diagonals)){ continue }
