...
import { DiagonalCollection, Diagonal } from "../../src/DiagonalCollection"

function getIrreducibleMorphismPlus(diagonal: Diagonal, move_diag_index: number, w: number, e: number, sort: boolean = true) {
    const N = (w+1)*(e+1)-2;
    ...
    if(nch.isWDiagonal(([new_diag[0],new_diag[1]] as Diagonal).sort(),w)){
        if(sort){
            new_diag.sort();
        }
        return new_diag;
